//******************************************************************************
// REPORT_ZSCORE object - A report object to display and navigate zScore results
//******************************************************************************
class Report_zScore extends Report {
	constructor(o) {
		super(o);
		let source = window.opener.zScore;
		...
		this.UI = {
			P: new RespTable({ID: "Ctrl_Pos", Fields: ["Name"], RowNumbers: true, Multiple: true, NoControls: true, Array: source.Controls.P, onSelect: this.computeZScore.bind(this)}),
			N: new RespTable({ID: "Ctrl_Neg", Fields: ["Name"], RowNumbers: true, Multiple: true, NoControls: true, Array: source.Controls.N, onSelect: this.computeZScore.bind(this)}),
			Plate: LinkCtrl.new("Select", {ID: "Plate_Select", Default: 0, List: this.Result.PlatesID, Label: "Plate", Title: "The result plate for which values will be displayed", Change: this.computeZScore.bind(this)}),
		}
		if(this.Result.PlatesID.length > 1) {this.UI.Plate.NavBar = true; this.UI.Plate.Lookup = {Active: false}}
		let b = LinkCtrl.button({Label: "Compute all", Title: "Click here to compute the z-score summaries for all plates", Click: function() {this.zScoreAllPlates()}.bind(this)});
		GetId("Plate_All").append(b);
		let b_pos = LinkCtrl.buttonBar([
			{Label: "Unselect all", Title: "Click here to unselect all positive controls", Click: function() {this.UI.P.setValue([]); this.computeZScore()}.bind(this)},
			{Label: "Select all", Title: "Click here to select all positive controls", Click: function() {this.UI.P.selectAll(); this.computeZScore()}.bind(this)},
		]);
		GetId("Ctrl_Pos").parentElement.prepend(b_pos);
		let b_neg = LinkCtrl.buttonBar([
			{Label: "Unselect all", Title: "Click here to unselect all negative controls", Click: function() {this.UI.N.setValue([]); this.computeZScore()}.bind(this)},
			{Label: "Select all", Title: "Click here to select all negative controls", Click: function() {this.UI.N.selectAll(); this.computeZScore()}.bind(this)},
		]);
	...
	computeZScore() { //Compute the z-score for selected items
		...
		this.getControlValues(plate).then(function(data) { //Collect values for this plate
			...
			let scores = this.processZscore(stats, data, plate); //Compute and display the z-score
	...
	processZscore(stats, data, plate) { //Process the stats for each controls in order to compute the z-scores
		let scores = [];
		data.Params.forEach(function(param, i) { //Process all parameters
			let section = Report.getBloc(this, param.Name).getSection("Z-scores");
	...
	zScoreTable(stats, data) { //Compute the z-scores and report the results as an HTML table
		...
		if(enough == false) {return {HTML: "<span class=\"warning\">Select at least one negative and one positive control to calculate a Z-score!</span>"}}
	...
	plateSummaryTables(data) { //Retrieve the tables that should be updated by the set of data received
		...
		data.Neg.forEach(function(p) {
			data.Pos.forEach(function(n) {
				tables.push({Title: Report_zScore.combinationName(n, p), Visible: (n.Selected && p.Selected)});
			});
		});
		return tables;
	}
