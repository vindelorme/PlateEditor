...
//*******************************************************************************
	...
	static form(I) { //Open a form to create or edit an area (Mode: create/edit), passing the necessary options
		...
		let RangeControls = { //Options for the ranges
			...
			Custom: LinkCtrl.new("Checkbox", {ID: range, Index: 3, Default: 0, Label: "Custom", Preserve: true, Change: function(v) {
		...
		Form.open({ //Open the form
			ID: id,
			HTML: "<div id=\"" + input + "\"></div><fieldset id=\"" + range + "\" style=\"display: none; margin-top: 5px\"><legend>Numbering</legend><div id=\"" + rangeOptions + "\"></div></fieldset>",
			Title: title,
			Buttons: buttons,
	...
	static load(table, areaData, plate, plateData) { //Load the provided areas data in the provided plate and table
		areaData.forEach(function(a) { //For each area to load
			if(a.Type == "Range") { //Create the object and log it in the table
				var targetArea = table.addRow(new Area({Name: a.Name, Color: a.Color, Type: a.Type, Replicates: a.Replicates, Direction: a.Direction, Priority: a.Priority, Custom: a.Custom}));
			}
			else {
				var targetArea = table.addRow(new Area({Name: a.Name, Color: a.Color, Type: a.Type}));
			}
